#!/usr/bin/env python3
""" ImageNet Training Dataset Script

This is intended to infer all kinds of results of the trained models on the ImageNet training set. 
- The training dataset is expected to be minimized -> 25 images per class. 
...
parser.add_argument('--data_dir', metavar='DIR', default="/home/bryan/miao/proximity/NeighborAgg/data/ILSVRC/Data/CLS-LOC",
                    help='path to dataset')
parser.add_argument('--output_dir', metavar='DIR', default="pytorch_image_models/intermediate_output/imagenet_train",
                    help='path to save the intermediate feature')
...
def validate(args):
    ...
    ds = create_dataset(name='torch/image_folder', root=args.data_dir, split=args.split, is_training=False, batch_size=args.batch_size)
    ...
    with torch.no_grad():
        ...
        # save_folder = "intermediate_output/imagenet_train"
        os.makedirs(args.output_dir, exist_ok=True)
        pickle.dump((ys, zs, logits, confs, preds), open(os.path.join(args.output_dir, 'out_{}.p'.format(args.model)), 'wb'))
